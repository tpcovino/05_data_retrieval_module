...
```{r}
gallatin <- whatNWISdata(parameterCd = param_cd, service = service_cd, stateCd = state_cd, startDate = start, endDate = end, countyCd = "Gallatin") %>% 
  select(site_no, station_nm, dec_lat_va, dec_long_va)

gallatin_map <- leaflet() %>% 
  addProviderTiles("OpenStreetMap") %>% 
  addAwesomeMarkers(data = gallatin, lat = ~dec_lat_va, lng = ~dec_long_va, label = ~station_nm, 
                    popup = ~paste("Site", site_no)) 
...
```{r}
gal_t_site <- data_avail_clean$site_no[1]
gal_t_param <- data_avail_clean$parm_cd[1]
gal_t_start <- data_avail_clean$begin_date[1]
gal_t_end <- data_avail_clean$end_date[1]


gal_t <- readNWISdata(sites = gal_t_site, parameterCd = gal_t_param, service ="dv", startDate = gal_t_start, endDate = gal_t_end) %>% 
  renameNWISColumns() %>% 
  select(site_no, dateTime, Wtemp_Max, Wtemp_Min, Wtemp)
...
```{r}
gal_t_q <- left_join(gal_t, gal_q, by = "dateTime")
...
```{r}
gal_t_q %>% 
  mutate(q_cms = Flow * 0.0283) %>% 
  ggplot(aes(x = q_cms, y = Wtemp, color = dateTime)) +
  geom_point() +
  labs(x = "Q (cms)", y = "T (degrees C)", color = "Date") +
...
```{r}
gallatin_sites <- gallatin$site_no

gallatin_q <- readNWISdata(sites = gallatin_sites, parameterCd = param_cd, service = "dv", startDate = start, endDate = end)

gallatin_q <- gallatin_q %>% 
  renameNWISColumns() %>% 
  select(site_no, date_time = dateTime, flow = Flow)

gallatin_q_names <- left_join(gallatin_q, gallatin, by = "site_no") %>% 
...
```{r}
gallatin_q_names %>% 
  ggplot(aes(x = date_time, y = flow_cfs)) +
           geom_point() +
